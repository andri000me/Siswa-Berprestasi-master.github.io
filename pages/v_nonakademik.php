<?php 
    ...
    function transformnonakademik($nonakademik) {
        if($nonakademik<70) {
            return 1;
        }else if($nonakademik>69 && $nonakademik<=79) {
            return 2;
        }else if($nonakademik>79 && $nonakademik<=89) {
            return 3;
        }else if($nonakademik>89 && $nonakademik<=100) {
            return 4;
        }
    }
    if($kelas && $idajaran) {
        ...
        //         $insert = mysqli_query($db,"INSERT INTO tbl_nonakademik(kehadiran, id_siswa, id_ajaran) VALUES('get[id_subkriteria]','$row[id_siswa]', '$idajaran')");
        //     }
        // }
        // echo $q;
    }
                        ...
                        <table class="table" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td width="20%">
                                        <select  name="nonakademik[]" class="form-control" required>
                                        <option value="">Input Nilai </option>
                                        <?php $q = mysqli_query($db, "select id_subkriteria,nama_subkriteria, nilai_subkriteria from tbl_subkriteria where id_kriteria='4'");
                                            while($item = @mysqli_fetch_array(@$q)) {
                                                if($item['id_subkriteria']==$row['nonakademik']) {
...
<?php 
    if(isset($_POST['simpan'])) {
      ...
      //  @$kehadiran = $_POST['kehadiran'];
        ...
        if($ss) {
            echo var_dump($nonakademik);
            for($i=0;$i<count($ss);$i++) {
                $cek = mysqli_query($db, "SELECT * from tbl_nonakademik where id_ajaran='$_GET[ajaran]'
                     and id_siswa='$ss[$i]'");
                if(mysqli_num_rows($cek)>0) {
                    // $query = "UPDATE tbl_nonakademik set nonakademik='$nonakademik[$i]', kehadiran='$kehadiran[$i]'
                    // where id_ajaran='$_GET[ajaran]' and id_siswa='$ss[$i]'";
                   $query = "UPDATE tbl_nonakademik set nonakademik='$nonakademik[$i]'
                    where id_ajaran='$_GET[ajaran]' and id_siswa='$ss[$i]'";
                } else {
                    // $query = "INSERT INTO tbl_nonakademik (id_ajaran,id_siswa,kehadiran,nonakademik) 
                    //             values('$_GET[ajaran]','$ss[$i]','$kehadiran[$i]','$nonakademik[$i]')";
                    $query = "INSERT INTO tbl_nonakademik (id_ajaran,id_siswa,nonakademik) 
                                values('$_GET[ajaran]','$ss[$i]','$nonakademik[$i]')";
                }
                
                mysqli_query($db, $query);
            }   
        }
        $url = BASE_URL."/index.php?p=nonakademik&ajaran=$_GET[ajaran]&kelas=$_GET[kelas]";;
        echo "<script>alert('Data Non Akademik berhasil Diupdate');
...
<script>
    function selectKelas(){
        var tahun = $("#tahun").val();
        var kelas = $("#kelas").val();
        var url = "<?=BASE_URL?>/index.php?p=nonakademik&ajaran="+tahun+'&kelas='+kelas;
        document.location.href=url;
    }
</script>
