<?php 
    function transformNilai($nilai) {
      ...
      function transformKehadiran($kehadiran) {
        if($kehadiran<56) {
            return 1;
        }else if($kehadiran>56 && $kehadiran<=70) {
            return 2;
        }else if($kehadiran>71&& $kehadiran<=85) {
            return 3;
        }else if($kehadiran>86 && $kehadiran<=100) {
            return 4;
        }
    }
    ...
    if($idajaran && $jurusan) {
        ...
        left join tbl_eskul eskul on eskul.id_siswa = tbl_siswa.id_siswa and eskul.id_ajaran='$idajaran'
         ...
         left join tbl_nonakademik nonakademik on nonakademik.id_siswa = tbl_siswa.id_siswa and nonakademik.id_ajaran='$idajaran'
        ...
        while($row = mysqli_fetch_array($siswa)) {
            ...
            $q5 = mysqli_query($db, "SELECT * FROM tbl_nonakademik where id_siswa='$row[alternatif]' and id_ajaran='$idajaran'")or die(mysqli_error($db));
            if(mysqli_num_rows($q5)>0) {
                $data = mysqli_fetch_assoc($q5);
                $res['nonakademik'] = $data['nonakademik'];
            
            }


             $q6 = mysqli_query($db, "SELECT * FROM tbl_eskul where id_siswa='$row[alternatif]' and id_ajaran='$idajaran'")or die(mysqli_error($db));
            if(mysqli_num_rows($q6)>0) {
                $data = mysqli_fetch_assoc($q6);
                $res['eskul'] = $data['eskul'];
            ...
            // // Save data Non Akademik
            $kriteria = mysqli_fetch_assoc(mysqli_query($db, "select * from tbl_subkriteria where id_kriteria='4' and id_subkriteria='$res[nonakademik]'"));
            $res['n_nonakademik'] = $kriteria['nama_subkriteria'];
            $res['c4'] = $kriteria['nilai_subkriteria'];
            $saveNilai = mysqli_query($db, "INSERT INTO tbl_penilaian_alt (id_alternatif,id_kriteria,id_subkriteria,nilai_subkriteria,id_ajaran) 
                values('$row[alternatif]','$kriteria[id_kriteria]', '$kriteria[id_subkriteria]', '$kriteria[nilai_subkriteria]','$idajaran')");

            // // Save data Eskul
            $kriteria = mysqli_fetch_assoc(mysqli_query($db, "select * from tbl_subkriteria where id_kriteria='5' and id_subkriteria='$res[eskul]'"));
            $res['n_eskul'] = $kriteria['nama_subkriteria'];
            $res['c5'] = $kriteria['nilai_subkriteria'];
            $saveNilai = mysqli_query($db, "INSERT INTO tbl_penilaian_alt (id_alternatif,id_kriteria,id_subkriteria,nilai_subkriteria,id_ajaran) 
                values('$row[alternatif]','$kriteria[id_kriteria]', '$kriteria[id_subkriteria]', '$kriteria[nilai_subkriteria]','$idajaran')");
        ...
        foreach($nilai as &$row2) {
            ...
            $row2['result'] = $row2['b1'] + $row2['b2'] + $row2['b3'] + $row2['b4']+ $row2['b5'];
            mysqli_query($db, "DELETE FROM tbl_hasil where id_siswa='$row2[id_siswa]' and id_ajaran='$idajaran'");

            mysqli_query($db, "INSERT INTO tbl_hasil(id_siswa, id_ajaran,nilai, rata_rata, kepribadian, kehadiran,nonakademik, eskul, jurusan )VALUES('$row2[id_siswa]','$idajaran','$row2[result]','$row2[mean]', '$row2[n_pribadi]', '$row2[n_kehadiran]','$row2[n_nonakademik]','$row2[n_eskul]', '$jurusan')");
        }
                        ...
                        <div class="col-md-4 my-auto">
                            <div class="form-group">
                                ...
                                <option value="">Pilih Jurusan</option>
                                    <option value="BKP" <?=@$_GET['jurusan']=='BKP'?'selected':''?>>BKP</option>
                                 <option value="DPIB" <?=@$_GET['jurusan']=='DPIB'?'selected':''?>>DPIB</option>    
                                    <option value="TG" <?=@$_GET['jurusan']=='TG'?'selected':''?>>TG</option>
                                 <option value="TITL" <?=@$_GET['jurusan']=='TITL'?'selected':''?>>TITL</option>    
                                    <option value="TPTU" <?=@$_GET['jurusan']=='TPTU'?'selected':''?>>TPTU</option>
                                 <option value="TP" <?=@$_GET['jurusan']=='TP'?'selected':''?>>TP</option>    
                                    <option value="TPG" <?=@$_GET['jurusan']=='TPG'?'selected':''?>>TPG</option>
                                 <option value="TKR" <?=@$_GET['jurusan']=='TKR'?'selected':''?>>TKR</option>    
                                    <option value="TSM" <?=@$_GET['jurusan']=='TSM'?'selected':''?>>TSM</option>
                                <option value="TSM" <?=@$_GET['jurusan']=='TSM'?'selected':''?>>TSM</option>    
                                    <option value="TKJ" <?=@$_GET['jurusan']=='TKJ'?'selected':''?>>TKJ</option>
                                <option value="RPL" <?=@$_GET['jurusan']=='RPL'?'selected':''?>>RPL</option>    
                                </select>
                            </div>
